 ...
 * @access	public
 */
	public function get($recursive = false, $full = false)
	{
		...
		if (isset($dir) && !empty($dir))
		{
			...
			while($entry = $dir->read())
			{
				...
				$exceptions = $full === false ? self::$nogood : self::$nogoodsimple;
 ...
 */
	public function refine($filter)
	{
		...
		foreach ($this->data as $key => $value)
		{
			if (is_a($value, 'dir'))
			{
				$filteredData = array_merge($filteredData, $value->refine($filter));
			}
			else
			{
				if (strstr($key, $filter)) $filteredData[$key] = $value;
